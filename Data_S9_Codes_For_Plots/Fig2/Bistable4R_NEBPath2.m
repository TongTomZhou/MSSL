...
%
%LISTv_Origin_Start=MATX_Points_Start(3,:)';
%LISTv_Zdirc_Start=MATX_Points_Start(3+4,:)'-MATX_Points_Start(3,:)';
%LISTv_Zdirc_Start=LISTv_Zdirc_Start/norm(LISTv_Zdirc_Start);
%LISTv_Xdirc_Start_=MATX_Points_Start(4+4,:)'-MATX_Points_Start(3,:)';
%LISTv_Ydirc_Start=cross(LISTv_Zdirc_Start,LISTv_Xdirc_Start_);
%LISTv_Ydirc_Start=LISTv_Ydirc_Start/norm(LISTv_Ydirc_Start);
%LISTv_Xdirc_Start=cross(LISTv_Ydirc_Start,LISTv_Zdirc_Start);
%LISTv_Xdirc_Start=LISTv_Xdirc_Start/norm(LISTv_Xdirc_Start);
%
%LISTv_Origin_End=MATX_Points_End(3,:)';
%LISTv_Zdirc_End=MATX_Points_End(3+4,:)'-MATX_Points_End(3,:)';
%LISTv_Zdirc_End=LISTv_Zdirc_End/norm(LISTv_Zdirc_End);
%LISTv_Xdirc_End_=MATX_Points_End(4+4,:)'-MATX_Points_End(3,:)';
%LISTv_Ydirc_End=cross(LISTv_Zdirc_End,LISTv_Xdirc_End_);
%LISTv_Ydirc_End=LISTv_Ydirc_End/norm(LISTv_Ydirc_End);
%LISTv_Xdirc_End=cross(LISTv_Ydirc_End,LISTv_Zdirc_End);
%LISTv_Xdirc_End=LISTv_Xdirc_End/norm(LISTv_Xdirc_End);
%
%for i=1:size(MATX_Points_Start,1)
%    MATX_Points_End(i,:)=transpose(inv([LISTv_Xdirc_End,LISTv_Ydirc_End,LISTv_Zdirc_End])*(MATX_Points_End(i,:)'-...
%                                        LISTv_Origin_End));
%    MATX_Points_Start(i,:)=transpose(inv([LISTv_Xdirc_Start,LISTv_Ydirc_Start,LISTv_Zdirc_Start])*(MATX_Points_Start(i,:)'-...
%                                          LISTv_Origin_Start));
%end
%
%tool.plot4RStruc(MATX_Points_Start,1,'c')
%tool.plot4RStruc(MATX_Points_End,1,'m')
%
%
...
for i=1:size(TSOR_Points_Motion,3)
    ...
    LISTr_Theta1(i)=theta1; LISTr_Theta2(i)=theta2; LISTr_Theta3(i)=-theta3; LISTr_Theta4(i)=theta4+pi;

    if i<=53
        LISTr_Theta2(i)=-theta2;
    end
    if i>=89
...
writematrix(MATX_Points_Motion,'MATX_Points_Motion_Bistable4R_FixedH3H4.csv');
...
function [TSOR_Points_Motion,fPhyProj]=ANS_Simulation_NEB(MATX_Points_Start,MATX_Points_End,INDX_FixedNodes,topoBarsStruc)
    ...
    K_nod=10^(0)*ones(numInvBars,1);   K_neb=10^(3); %0.1,1.5
    step_m=0.01;
    ...
    numConfigBalls=100; diffConfig=configC2-configC1; locListConfigBall=[];
